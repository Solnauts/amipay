...
//Transfer from sender usdc ata ==> net amount to receiver usdc ata, fee to main vault
#[derive(Accounts)]
pub struct TransferToVault<'info> {
    ...
    //sender usdc account (program-owned ATA for the sender)
    #[account(mut, token::authority = main_state_account, token::mint = usdc_mint)]
    pub sender_usdc_ata: InterfaceAccount<'info, TokenAccount>,

    //receiver usdc account (program-owned ATA for the receiver)
    #[account(mut, token::authority = main_state_account, token::mint = usdc_mint)]
    pub receiver_usdc_ata: InterfaceAccount<'info, TokenAccount>,
...
impl<'info> TransferToVault<'info> {
    ...
    //transfer net amount to receiver and fee to main vault
    pub fn transfer_to_receiver_and_vault(&self, amount: u64) -> Result<()> {
        let decimals = self.usdc_mint.decimals;

        let usdc_mint = self.main_state_account.usdc_mint;
        let admin = self.signer.key();

        // PDA signer seeds for main_state_account (authority over all ATAs)
        let seeds = [
            b"main_state",
            usdc_mint.as_ref(),
            admin.as_ref(),
        ...
        // 1) Transfer net amount: sender_usdc_ata -> receiver_usdc_ata
        let cpi_accounts_receiver = TransferChecked {
            mint: self.usdc_mint.to_account_info(),
            from: self.sender_usdc_ata.to_account_info(),
            to: self.receiver_usdc_ata.to_account_info(),
            authority: self.main_state_account.to_account_info(),
        };

        let cpi_context_receiver = CpiContext::new_with_signer(
            self.token_program.to_account_info(),
            cpi_accounts_receiver,
            signer_seeds,
        );

        token_interface::transfer_checked(cpi_context_receiver, net_amount, decimals)?;

        // 2) Transfer fee: sender_usdc_ata -> main_usdc_vault
        ...
        // Re-derive signer seeds (borrow checker requires fresh reference)
        let seeds_fee = [
            b"main_state",
            usdc_mint.as_ref(),
            admin.as_ref(),
